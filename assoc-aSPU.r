...
#---------all kernel R scripts put here------------------------------#
source_file_PWD <- sprintf("%s/R/",programRootPWD)
#
...
if (exists("cl")) {
	clusterExport(cl, c("source_file_PWD", "programRootPWD") )
	clusterEvalQ(cl, {
											setwd(source_file_PWD)
											source("transform.r")
											source("aspuTest_parralled_withSco_RV_version_updatedScoreTest_within.r")	# load SPU3 func.
...
message("Data loaded, start computation!")
if (!include_aSPU) {	# only 5 asymptotic tests, i.e., sum, SSU, SSUw, Score and UminP.
		parallelFlag <- FALSE
			#--run---#
		testResult <- foreach(iter = iterObj,.errorhandling = "pass",.verbose=F) %dopar% {	# since asymptotic tests are fast, we prefer to parallel over genes.
...
# }
	if (!parallel_over_gene) {	
		parallelFlag <- TRUE
		#--run---#
		# testResult <- foreach(iter = iterObj,.errorhandling = "pass",.verbose=F) %dopar% {
		testResult <- foreach(iter = iterObj,.errorhandling = "pass",.verbose=F) %do% {	# gene level in serial, within each gene, the computation is paralleled.
